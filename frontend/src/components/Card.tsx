...
export const Card = forwardRef<HTMLDivElement, CardProps>(
  ({ card, className, style, ...props }, ref) => {
    ...
    const bgColor = isHidden ? '#333' : (card.color === 'black' ? '#222' : card.color);
    ...
    return (
      <motion.div
        ref={ref}
        className={cn(
          "absolute rounded-xl shadow-lg border-2 select-none overflow-hidden bg-white flex items-center justify-center cursor-grab active:cursor-grabbing shrink-0",
          className
        )}
        style={{ 
      ...
      >
        <div className="absolute inset-2 rounded-lg flex flex-col items-center justify-center" style={{ backgroundColor: bgColor, color: textColor }}>
          {isHidden ? <span className="text-4xl font-bold opacity-50">UNO</span> : (
            <><span className="text-xs uppercase absolute top-2 left-2">{card.color}</span><span className="text-lg font-bold">{card.card_type}</span><span className="text-5xl font-black">{card.value ?? '★'}</span></>
          )}
        </div>
